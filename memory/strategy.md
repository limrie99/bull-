...
## Cash-deployment / market-floor policy (added 2026-08-07 — Lauren APPROVED Option B)
...
- **Target:** move total cash from ~75% toward the **50–55%** band. Initial tranche ≈ **20% of equity** (moves us to ~55% cash); a further tranche toward ~50% only after the first settles and if still idle. Never deploy so much that total cash falls below the **10–20% minimum buffer**.
...
- **The ≥70 Conviction gate is UNCHANGED and was NOT lowered.** The floor is explicitly the alternative to loosening the quality bar: idle cash matches the market via the index while the stock sleeve keeps requiring 2+ signals AND Conviction ≥70. We do not force-buy mediocre individual names to deploy cash.
- **Execution:** build the floor at the **next market-open routine (Mon 2026-08-10)**. Compute exact shares at the live open price; log the fill to trade-log.md like any buy (note "MARKET-FLOOR / index sleeve, no stop"). If Lauren countermands over the weekend (inbox), stand down.
