...
- **High-beta AI/semis entry discipline (added 2026-06-05, advisory — NOT a guardrail change).** NVDA round-tripped to a stop-out *twice* in five weeks (5/4 −$159, 6/5 −$849), both times on macro/sector moves with the thesis fully intact. Root cause is beta, not the thesis: a 2–3× beta name carrying a −7% hard stop gets shaken out on a normal macro pullback before the fundamentals can express, especially when entered within a few days of a sharp run or into a known event (NFP). So for high-beta AI/semiconductor names: **size as a smaller starter (≤10%, not the full 12–15%)** and **prefer entering on a confirmed base, not chasing a multi-day spike or initiating fresh beta the day before a binary macro print.** This does not touch any guardrail (the −7% stop and 10% trailing stay exactly as-is). It is a 2nd-occurrence refinement on **probation**: a 3rd same-pattern stop-out escalates this to a hard sizing/entry rule.
  ...
  - **Update 2026-09-04 — the ENTRY-TIMING half is TESTED for the first time; it RECURRED; scope BROADENED, still advisory-on-probation.** SNPS — a Grade-A (~80) EDA/chip-design *software* name that is **high-multiple/long-duration (P/E >70) and therefore acutely RATE-sensitive** — was initiated fresh on **Thu 9/3, the session BEFORE Friday's binary August NFP**, and **−7%-hard-stopped the very next morning** (9/4, −$708 / −7.07%) when a HOT jobs number (~162k vs ~53k expected) spiked the 10Y toward multi-year highs and flushed long-duration software. This is the **2nd occurrence of the fresh-entry-into-a-binary-macro-print stop-out pattern** (1st: NVDA entered 6/1 → −7%-stopped 6/5 on the post-NFP semis flush) and the first real test of the entry-timing half since it was written. Two surgical refinements: **(a) SCOPE — the advisory was written for "high-beta AI/semiconductor" names, but SNPS is high-multiple/rate-sensitive *software*, a bucket a literal reading could exclude; that scoping gap is exactly why the entry cleared. Broaden the advisory to cover high-beta AI/semis OR high-multiple/long-duration rate-sensitive names when a rate-relevant binary macro print (NFP / CPI / FOMC) is imminent.** (b) STATUS — **still ADVISORY, not a guardrail:** the *sizing* rule already did its job (~10% starter → −$708, not −$1,500+ at full Grade-A size), and per the 6/5→7/3 graduation precedent the sizing half needed 3 occurrences before it hardened, so the entry-timing half stays on **probation at occurrence 2**. **Pre-commit: a 3rd same-pattern fresh-entry-into-a-binary-macro-print stop-out graduates the entry-timing half to a HARD rule** (no fresh rate-sensitive/high-beta beta initiated the session before a known binary macro print; wait for the print + a stabilize/reclaim). Honest note: the deeper lesson is **DISCIPLINE, not a rulebook gap** — the 9/3 pre-market explicitly flagged the hot-NFP whisper (+65–95k) and ~64–74% odds of a Sept HIKE, and we initiated anyway. Future-Bull should treat this broadened advisory as a **near-veto, not a soft preference.**
...
## Operating model — voices, continuity, and research fan-out (added 2026-09-21, Herk-inspired)

**This is a process/documentation change, not a rule change.** Nothing in this section touches a guardrail: `mode: paper`, the −7% hard stop, the 10% trailing stop at +5%, the ≥70 Conviction gate, the 2-signal rule, max 5 conviction names, max 3 new buys/week, the ~$10k (~10%) cash floor, and the index-sleeve stop carve-out are all exactly as written above. Nothing here enables live trading.

**Source (credit where it's due):** Nate Herk, *"I Turned GPT-6 Astra Into a 24/7 Stock Trader"* (Sep 2026) — https://youtu.be/TLQLfa7yH4I — plus his follow-up pattern: when Astra turned out to be *blocked from placing trades*, he kept Astra as the **strategist** and handed execution to a separate "Trader" bot. Lauren asked (inbox, 2026-09-21) to fold the useful parts of that architecture into Bull. We took the architecture. We deliberately did **not** take the day-trading cadence — see *Schedule mapping* below.

### Role split — three voices, one accountable owner

Herk's accident is our design principle: **the voice that has an idea should not be the voice with its hands on the order button.** Bull already works this way (sub-agents research, Bull executes); naming it makes the logs read like a committee instead of a monologue, and makes "we researched it and chose not to buy" a first-class, auditable outcome.

| Voice | Owns | Never does |
|-------|------|------------|
| **Research / Strategist** | Overnight + macro digests, theses, the 5 Conviction sub-scores, watchlist ordering, buy/sell *proposals* | Never places, sizes, or cancels an order. A proposal is a recommendation, never a fill. |
| **Risk** | Sizing bands, the high-beta ≤10% starter cap, the binary-macro-print entry advisory, stop integrity, daily loss cap, the cash floor | Never originates a thesis to justify a trade it already wants. |
| **Trader / Execution** | The only voice that touches Alpaca: orders, bracket stops, trailing-stop swaps, fill confirmation, the SPY floor sleeve | Never buys a name that has not cleared Research's gate (2+ signals AND Conviction ≥70) and Risk's checks. |

- **Bull is the accountable owner and the single voice Lauren hears.** The voices are how the work is organized and logged, not a committee she has to referee.
- **A proposal is never self-executing.** Research proposes → Risk checks → Trader re-validates at live prices and either fills or declines **with the reason logged**. "Research liked it, Trader declined on a wide spread / a stale score / no open slot" is a legitimate outcome — that separation is the whole point.
- **Naming collision, flagged deliberately:** the `astra/` folder in this repo is a *different thing* from Herk's Astra. It is league strategy #3 (`gpt-6-astra`, its own paper account, its own code-enforced guardrails) and it is **blind to Bull's decisions by experimental design**. Do **not** wire Astra's proposals into Bull's order flow — that would contaminate the cross-agent comparison. Bull's strategist→executor handoff happens *between Bull's own voices*, never across strategies.

### Speaker labels in logs and messages

- **`memory/research-log.md`** — when the voices disagree, or a proposal is declined, write the exchange with labels (`**Research:** …` / `**Risk:** …` / `**Trader:** …`) so future-Bull can see *where* the decision was made, not just the outcome.
- **`memory/messages.md` and Telegram** — stay **one warm Bull voice**. Lauren should never have to track personas to find out whether we are up or down. A labelled line is allowed inside the *Why* section when the disagreement is the story ("my research side liked Thermo Fisher; my risk side said there was no slot worth freeing, so we passed") — plain English, no org-chart jargon.

### Continuity loop — every run reads the last handoff and writes the next one

Each scheduled run wakes stateless, so the handoff *is* the memory. Every routine:

1. **Reads** `CLAUDE.md`, this file, `portfolio.md`, `watchlist.md`, `inbox.md` (Pending), the **most recent `research-log.md` entry — that is the previous slot's handoff** — and the last 10 trades.
2. **Verifies against reality before acting** — live Alpaca account, positions, open orders, and `/v2/clock`. The handoff says what we *planned*; Alpaca says what is *true*. Truth wins, and any gap gets logged rather than quietly reconciled.
3. **Acts only within its slot's mandate** (table below).
4. **Writes the handoff** — end every research-log entry with an explicit **`Handoff → <next routine>`** block: open standing actions, the price levels or triggers being watched, what would change the plan, and anything left unverified. **Standing actions carry forward verbatim until executed or cancelled** — restate them in every handoff until they clear. (This is exactly how the 8/7 cash-deploy decision survived until it was executed.)

### Research fan-out — allowed process, not a daily obligation

Fan out parallel sub-researchers (one per ticker, plus macro / earnings / sector / smart-money) in the **research-heavy** work: pre-market, the weekly review, and any run that rebuilds the watchlist or proposes a strategy change. Then synthesize the digests into one decision. Two limits: (a) it is **not required** in the tight slots (open / midday / close) — running a full fan-out to justify inaction is waste; (b) sub-agents return evidence and scores, never orders — execution stays with the Trader voice.

### Secrets stay in the environment

Keys live in `.env` locally and in **GitHub Actions secrets** for anything cloud-scheduled. Never in chat, never in `inbox.md`, `messages.md`, the research log, or a commit. If an API error echoes a key, redact it before logging. A run missing a required key halts and says so (per `CLAUDE.md`) — it never guesses or fabricates a fill.

### Schedule mapping — what we took, and the one thing we refused

Bull keeps its existing five CT slots, unchanged:

| Bull slot (CT) | Mandate |
|----------------|---------|
| 6:00 pre-market | Research fan-out, Conviction re-scores, watchlist re-rank. No trades. |
| 8:30 open | Execute the plan at live prices; place bracket stops. |
| 12:00 midday | Manage what's on — stop integrity, trailing-stop swaps, thesis breaks. |
| 3:00 close | Mandatory plain-English scorecard vs SPY, plus risk check into the overnight. |
| Fri 4:00 weekly review | Honest retro, alpha vs SPY, strategy changes with a dated changelog entry. |

Herk's build wakes six times a market day and **flattens the book by mid-afternoon** so nothing is held overnight. **We do not adopt that.** Our slots exist to *invest and manage risk*, not to be flat by the bell. This is a swing/position book (holds days to months) whose overnight risk is handled by stops and sizing — not by selling everything at 3:50. Closing daily would tax every thesis before it can express (the same mistake v1's 2% stops made) and would make beating SPY arithmetically hard, since SPY is never flat either. The six-slot ET schedule in `astra/` belongs to that separate experiment, and even there the 15:50 "risk-shutdown" means *no new buys plus a journal*, not a forced liquidation.
...
- **2026-09-21 (per Lauren, inbox: fold Nate Herk's Astra architecture into the strategy) — PROCESS/DOCS CHANGE ONLY, no guardrail touched.** Added the "Operating model — voices, continuity, and research fan-out" section above, adapted from Nate Herk's *"I Turned GPT-6 Astra Into a 24/7 Stock Trader"* (Sep 2026, https://youtu.be/TLQLfa7yH4I) and his follow-up pattern where Astra — blocked from placing trades — became the **strategist** and a separate "Trader" bot executed. What we took: (1) the **three-voice role split** (Research/Strategist proposes and scores; Risk checks sizing, stops, and the macro-print advisory; **Trader is the only voice that touches Alpaca**) with speaker labels in `research-log.md` and a single warm Bull voice preserved in `messages.md`/Telegram so Lauren never has to referee personas; (2) an explicit **continuity loop** — read strategy + the previous slot's handoff + *live* account state, act only within this slot's mandate, then write a `Handoff → <next routine>` block, with standing actions restated verbatim until executed or cancelled; (3) **research fan-out** sanctioned for strategy/watchlist rebuilds as well as pre-market, with sub-agents returning evidence and scores but never orders; (4) **secrets stay in `.env` / GitHub Actions secrets**, never in chat, inbox, messages, logs, or commits (already true — now written down). What we deliberately REFUSED: Herk's **six day-trade slots and his flatten-the-book-by-mid-afternoon cadence.** Our five CT slots stay as they are and exist to invest and manage risk, not to end the day flat — this is a swing/position book (days to months) where overnight risk is handled by stops and sizing; daily liquidation would tax every thesis before it can express (v1's 2%-stop mistake) and makes beating a never-flat SPY arithmetically hard. Also flagged the **naming collision**: this repo's `astra/` is league strategy #3, blind to Bull by experimental design — its proposals must NOT be wired into Bull's order flow; Bull's strategist→executor handoff is internal, between its own voices. What this deliberately does NOT change: **every guardrail is untouched** — `mode: paper` (no live trading enabled), the −7% hard stop, 10% trailing at +5%, ≥70 Conviction gate, 2-signal rule, max 5 conviction names, max 3 buys/week, the ~$10k/~10% cash floor, and the index-sleeve stop carve-out. Paper only; this edit places no Alpaca order.
- **2026-09-04 (weekly review) — REFINEMENT, NOT a guardrail change: broadened the high-beta entry-timing ADVISORY to cover high-multiple/long-duration rate-sensitive names, and logged SNPS as the 2nd occurrence of the fresh-entry-into-a-binary-macro-print pattern (still advisory-on-probation).** Trigger: SNPS (Grade-A ~80 EDA/chip-design *software*, P/E >70 = acutely rate-sensitive) was initiated fresh Thu 9/3, the session before Friday's binary August NFP, and −7%-hard-stopped the next morning (−$708 / −7.07%) on the post-NFP rate shock — NOT a thesis break (beat-and-raise intact; verified sector/rate noise). This is the 2nd occurrence of the pattern the 6/5 learning named (1st: NVDA 6/1→6/5, post-NFP semis flush) and the first test of the entry-timing half since it was written. What changed: (1) **SCOPE** — the advisory was scoped to "high-beta AI/semiconductor," but SNPS is high-multiple/long-duration rate-sensitive software, which a literal reading could exclude (that gap is why the entry cleared); it now covers high-beta AI/semis OR high-multiple/long-duration rate-sensitive names into a rate-relevant binary print (NFP/CPI/FOMC). (2) **STATUS** — it stays **ADVISORY on probation at occurrence 2**, with a **pre-commit that a 3rd same-pattern stop-out graduates it to a HARD rule** (mirroring how the *sizing* half graduated advisory→standing on 7/3 after its 3rd occurrence). What this deliberately does NOT change: **no guardrail** — the −7% hard stop, 10% trailing, ≥70 Conviction gate, 2-signal rule, position/weekly caps, the high-beta ≤10% sizing rule, and the cash-floor policy are all untouched (the stop and the sizing rule both worked exactly as designed on SNPS — the loss was capped at ~10%-position size). I deliberately did NOT hard-code a rule on the 2nd occurrence, because the graduation precedent requires 3 and reacting to one bad week by hardening a rule is the over-tuning this strategy warns against. The honest lesson is discipline — follow the advisory we already had (we had foreknowledge: the 9/3 pre-market flagged the hot-NFP whisper and ~64–74% Sept-HIKE odds) — treated now as a near-veto, not a soft preference. Paper only; this edit places no Alpaca order.
